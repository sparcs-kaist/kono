...
export default ({ 
    ...
    if (useDynamicPositioning) {
        ...
        let gridRows, gridRowSizes, gridColumns, gridColumnSizes, imageWidthCoeffs, imageHeightCoeffs;
        switch (numImages) {
            case 1:
                gridRows          = [1]
                gridRowSizes      = [2]
            ...
            <div className={styles.ImageGridPanel}>
            {
                [...Array(gridNumCells).keys()].map(idx => (
                    <ImageThumbnailPanel
                        ...
                        imageWidth={gridCellSize * imageWidthCoeffs[idx]}
                        imageHeight={gridCellSize * imageHeightCoeffs[idx]}
